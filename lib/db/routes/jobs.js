...
router.get('/employee/:id', /*checkJwt, jwtAuthz(['read:info']), */function (req, res) {
    _index2.default.sequelize.query("exec GetJobsByEmployee :empID;", { replacements: { empID: req.params.id }, type: _index2.default.sequelize.QueryTypes.SELECT })
    // db.sequelize.query("SELECT j.JobName FROM job j INNER JOIN employee_job ej ON j.JobID = ej.JobID WHERE ej.EmployeeID = " + req.params.id + ";")
    .then(function (jobs) {
        console.log(jobs);
        // var jobsResult = jobs.map( el => {
        //     return el.JobName;
        // })
